...
import org.firebrand.dao.utils.ObjectUtils;
...
public class ClassMetadata<T> {
    ...
    private void addColumnToColumnFamilyDefinition(Column colAnnotation, String property, boolean indexed) throws ClassNotFoundException {
        if (!property.equals(keyProperty)) {
            ...
            columnDef.setValidation_class(defaults ? org.firebrand.dao.annotations.Column.DEFAULTS.VALIDATION_CLASS.getName() : colAnnotation.validationClass().getName());
            indexed = indexed || isMappedContainer(property);
            if (!indexed) {
                indexed = defaults ? org.firebrand.dao.annotations.Column.DEFAULTS.INDEXED : colAnnotation.indexed();
            }
            if (indexed) {
                columnDef.setIndex_name(String.format("%s_%s_%s", keySpace, columnFamily, property));
                columnDef.setIndex_type(defaults ? org.firebrand.dao.annotations.Column.DEFAULTS.INDEX_TYPE : colAnnotation.indexType());
            }
    ...
    private void processSimpleColumn(Field element, String propertyName) throws ClassNotFoundException, IntrospectionException {
        ...
        org.firebrand.dao.annotations.Column colAnnotation = element.getAnnotation(org.firebrand.dao.annotations.Column.class);
